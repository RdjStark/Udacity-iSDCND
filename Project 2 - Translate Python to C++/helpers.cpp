...
vector < vector <float> > normalize(vector < vector <float> > grid) {
	...
	float total = sum(newGrid);

	for (int row = 0; row < newGrid.size(); row++) {
		for (int column = 0; column < newGrid[0].size(); column++) {
